...
void DelayLine::pushSamples(const juce::AudioBuffer<float>& srcBuffer, int nSam, int nCh)
{
    ...
    for (int i = 0; i < nCh; ++i)
        juce::FloatVectorOperations::copy(delayBuffer.getWritePointer(i) + index, srcBuffer.getReadPointer(i), b1);

    if (b2 > 0)
        for (int i = 0; i < nCh; ++i)
            //Copy rest of buffer to delayBuffer
            juce::FloatVectorOperations::copy(delayBuffer.getWritePointer(i), srcBuffer.getReadPointer(i) + b1, b2);
}

void DelayLine::readSamples(juce::AudioBuffer<float>& dstBuffer, int nSam, int nCh)
{
    //b1 = samples before wrap, b2 = samples after wrap
    int index, b1, b2;
    getReadBlockSize(nSam, index, b1, b2);
    for (int i = 0; i < nCh; i++)
        juce::FloatVectorOperations::copy(dstBuffer.getWritePointer(i), delayBuffer.getReadPointer(i) + index, b1);

    if (b2 > 0)
        for (int i = 0; i < nCh; i++)
            juce::FloatVectorOperations::copy(dstBuffer.getWritePointer(i) + b1, delayBuffer.getReadPointer(i), b2);
}
